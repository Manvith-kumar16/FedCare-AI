"""
FedCare AI Hospital — XAI Service
Local SHAP explanations using TreeExplainer.
"""
import os
import io
import base64
...
from app.services.ai_service import load_local_model, load_global_model
...
def _generate_shap_waterfall(
    ...
    try:
        ...
        print(f"[local-xai] SHAP plot error: {e}")
        return None
...
def generate_shap_explanation(
    ...
    """
    Generate local SHAP values and visualizations on the hospital node.
    """
    # 1. Load active model (local model preferred, fallback to global)
    model = load_local_model(server_id, hospital_id)
    if model is None:
        model = load_global_model(server_id)
        
    if model is None:
        return {"error": "No model found. Please train local or download global model."}
    ...
    try:
        # Check if the model is the custom ensembled classifier
        if isinstance(model, FederatedEnsembleClassifier):
            # Compute SHAP values of each sub-booster and average them
            all_vals = []
            base_vals = []
            ...
            # Standard XGBClassifier TreeExplainer
        ...
        # Build output importance mapping
        ...
        print(f"[local-xai] SHAP explanation error: {e}")
        return {"error": str(e)}


def generate_local_feature_importance(
    server_id: int,
    hospital_id: int,
    feature_columns: List[str]
) -> Dict:
    """
    Compute local model feature importances.
    """
    model = load_local_model(server_id, hospital_id)
    if model is None:
        model = load_global_model(server_id)
    ...
    try:
        if isinstance(model, FederatedEnsembleClassifier):
            # Take average of constituent models' importances
            importances = np.zeros(len(feature_columns))
            for sub_model, w in zip(model.models, model.weights):
                importances += sub_model.feature_importances_ * w
        elif isinstance(model, LogisticRegression):
            # For Linear models, take absolute coefficient values as raw importance
