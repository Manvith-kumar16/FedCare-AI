"""Dataset metadata model (hospital local)"""
...
class Dataset(Base, TimestampMixin):
    ...
    hospital_id: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    server_id: Mapped[int] = mapped_column(Integer, nullable=False)
    filename: Mapped[str] = mapped_column(String(255), nullable=False)
    file_path: Mapped[str] = mapped_column(String(500), nullable=False)
    row_count: Mapped[int] = mapped_column(Integer, default=0)
    feature_count: Mapped[int] = mapped_column(Integer, default=0)
    columns: Mapped[str] = mapped_column(Text, nullable=True)  # JSON list of columns
