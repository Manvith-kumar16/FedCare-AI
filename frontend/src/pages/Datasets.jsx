...
import { getDatasets, getDatasetStats, getDatasetPreview, uploadDataset, startTraining, getTrainingStatus, clearDatasets } from '../api'
...
export default function Datasets() {
  ...
  useEffect(() => {
    let interval
    if (simulationActive && simStatus !== 'Completed') {
  ...
  async function handleFileUpload(e) {
    ...
    try {
      await uploadDataset(1, file)
      addToast('Data uploaded successfully! Initializing Federated Learning...', 'success')
      
      await loadData() // Refresh list with real data

      // Trigger Actual Backend Training
      await startTraining({ server_id: 1, num_rounds: 5, local_epochs: 5 })
